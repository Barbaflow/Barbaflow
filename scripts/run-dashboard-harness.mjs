/**
 * Executa o harness do dashboard operacional do modo offline. Usa o Vite já
 * instalado apenas para resolver o alias `@` e compilar TypeScript sob demanda
 * (ssrLoadModule) — nenhuma dependência nova.
 *
 *   node scripts/run-dashboard-harness.mjs
 */
...
async function main() {
  ...
  try {
    const mod = await server.ssrLoadModule("/src/mocks/__harness__/dashboard-harness.ts");
